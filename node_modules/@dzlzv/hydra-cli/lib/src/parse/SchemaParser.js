...
class GraphQLSchemaParser {
    ...
    getUnifiedSchema(schemaPath) {
        let schemaString = '';
        if (fs.lstatSync(schemaPath).isDirectory()) {
            fs.readdirSync(schemaPath).forEach((file) => {
                if (fs.lstatSync(path_1.default.resolve(schemaPath, file)).isFile() &&
                    path_1.default.extname(file) === '.graphql') {
                    schemaString = schemaString.concat(fs.readFileSync(path_1.default.resolve(schemaPath, file), 'utf8'), '\n\n');
                }
            });
        }
