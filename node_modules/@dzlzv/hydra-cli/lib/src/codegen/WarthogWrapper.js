...
class WarthogWrapper {
    constructor(flags, prod = true) {
        ...
        if (!fs.existsSync(this.schemaResolvedPath)) {
            throw new Error(`Cannot open the schema file or folder: ${this.schemaResolvedPath}. Check if it exists.`);
        }
    }
    async run() {
        const skipIfNoDeps = () => {
    ...
    prepareProjectFiles() {
        ...
        pkgFile.dependencies.warthog = utils_1.getWarthogDependency();
    ...
    async codegen() {
        ...
        utils_1.parseWarthogCodegenStderr(stderr);
        if (this.prod)
